...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ── Raw text passthrough (allowlisted domains only) ───────────────────────
    // For non-RSS feeds (e.g. abuse.ch CSV blocklists) that the browser cannot
    // fetch directly because the upstream sends no CORS headers.
    if (rawParam) {
      let rawUrl: string;
      try {
        rawUrl = decodeURIComponent(rawParam);
      } catch {
        return new Response(JSON.stringify({ error: "Invalid raw URL encoding" }), {
          status: 400,
